...
| **laya / reflex Mac 参考学习** | ✅ 完成（2026-09-19） | **域外崩塌是这一类模型的固有行为，非 NanoJev 缺陷**：laya 自述基座零样本"接近随机"、"是待专门化的快速基座"。同 13 题实测：laya 0.9 下答 3/13（中位 0.656、最大 1.000）vs 我们 0/13——**但同样的安全关键错误它答得更自信**（`safe_to_drop`=true@0.878、`blocked`=false@1.000）→ **换更强模型是把"弃权"变成"自信答错"，更糟**。laya 的 Router/工作流预设/分组温度校准值得吸收。见 [学习文档](LAYA_REFLEX_MAC_STUDY_V1.md) |
| **参考集补漏 + 读法实验**（HF） | ✅ 完成（2026-09-19） | **发现 1770⭐ 的 `TheoLeeCJ/SemIf`**（前身 OpenJev，作者即反对压缩的 @theo）及其 CPU 移植 `Meanblock/JEV-CPU`——**用与我们相同的 Qwen3-0.6B、笔记本 CPU、跨八个域（含 code-review triage、incident severity）**；另有 `open-jev-deberta-v3-large`（**支持 255 选项**）、`jev-schema-scorer`（**criteria 推理时读取、无需重训**）。**架构分野明确：任务拟合头（我们）vs schema 条件化 logits 读法（它们，声称无需重训即可换任务）。** ⚠️ 实测把 SemIf 读法套到我们 checkpoint 上：中位置信度 0.505→**0.899**、0.9 下作答 0→**6/13**，但**排列对照证明是纯位置伪影**（所选字母 13/13 恒为 A，所选描述 0/13 不变）——**不是修复**。正确对照是用基座 Qwen3-0.6B（下载中） |
| **无模型安全去重** | ✅ 完成（**负面**，40 项测试） | 在仓库全部真实 fixture 上 **0 删除 / 0 字符 / 0 token**（A4 11 例 + 最大语料 2400 例）；且**未改动的网关根本无法承载**该臂（`removal_plan` 只接受 `high_irrelevance_score`，否则 fail-open）。**"不依赖模型的 token 节省"这条路也关闭了** → 在模型闸门可用前，必须停止宣称任何节省 |
| **域适应 runbook** | ✅ 交付（未训练） | 阻塞 A **已解除**：F1 交付语料 **41 对 / 246 条**（train 108/dev 42/calib 42/test 54，7 家族，25 项测试，`validate_training_row` 全过），且**翻转证明改为计算而非假设**（修掉一个真实的优先级 bug）。**未达 120 对目标**——作者拒绝注水，如实说明需再一轮人工编写。剩余阻塞：**B 适配器**（已给出 trainer_view，待评审）、**C 协议 §7 不可执行**（无 peft、MPS 拒 BF16）→ 全骨干 FP32 属**协议变更需重新评审**。算力实测便宜：**1.41 s/步**、峰值 **5.26 GB** |
