...
**所以这不是数据质量问题，是策略对输入混沌。** 三个放大器已定位，必须在任何金融数字有意义之前修掉：① 拒单/部分成交静默破坏仓位路径；② 数量按当前价重算使仓位随路径复利；③ 容量政策与场所报告成交量耦合（Aster 日均 BTCUSDT 成交量中位数 9,959 vs Binance 128,882）。这已提升为 Track B 的**首道门禁 B0**。
...
**执行**（三项放大器 + 一项强制报告）：
1. **让成交偏离成为错误而非静默状态变化**：订单被拒、部分成交或过期时，harness 必须抛错或显式对账预期仓位与实际仓位；**禁止**从假定仓位继续。
2. **仓位规模默认声明式且与路径无关**：在信号路径内按当前权益复利会把 1 bp 的输入差异放大成 2.7 倍的输出差异，摧毁可测量性。复利只能作为**显式且单独报告**的选项。
3. **容量与成本参数不得与场所报告成交量耦合**（除非该耦合本身是研究对象），否则跨场所比较无意义。
4. **强制敏感性报告**：每个金融结果都必须同时给出同一策略在扰动种子 / 日期集 / 场所下的离散度。**单个数字不得作为结果呈现。**

**验收**：跨至少三场所、同区间同策略的结果离散度小到可解释，**或** harness 明确声明无法给出并拒绝输出头条数字。
