...
| 模型闸门能力（Track A） | 🟡 Amber | 网关/可逆过滤/A4 已验证，但真实 checkpoint 提议 0 删除、`MAX_SCORED=32` 长上下文整体 bypass、Catalog Choice 受干扰 100%→54.17%；**skill：域外工程题 13/13 弃权（0.9 门限，观测），域内迷宫题中位置信度 0.779、≥0.9 者 97.8% 正确** → 崩塌是**域外**现象；已加短语范围守卫（见 [就绪度实测](NANOJEV_SKILL_READINESS_V1.md)、[域外诊断](SKILL_ABSTENTION_DIAGNOSIS_V1.md)） |
