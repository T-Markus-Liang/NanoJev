...
| **温度重标定修复尝试** | ✅ 完成（**负面结论**） | 仅在校准集拟合得 **0.854**；准确率处处不变（温度不改 argmax），ECE 在 test 改善 −0.031 却在 ood 恶化 +0.018、校准集/dev 均恶化 → **不泛化**；调查题置信度 0.505→0.515，**仍全部低于 0.9**。故"重标定即可修好弃权"这条路**已关闭** |
