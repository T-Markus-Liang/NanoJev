...
真实数据上同一参考策略产出**四个互相矛盾**的结果：Binance 修复前 −49,920.90、修复后 +21,173.77、Bybit −77,919.27、Aster +194,025.96。**跨场所极差 271,945，是本金的 2.7 倍**，而三场所 mark 价只差 **1–2 个基点**（Aster vs Bybit 平均绝对偏差 0.0199%，Aster vs Binance 0.0122%）。

**价格水平差异不足以解释这个极差**——但这不排除数据问题：时间对齐、单位、资金费处理、成交量口径与缺测尚未逐一排除审计。已定位的**候选机制**在执行路径上，三个放大器必须在任何金融数字有意义之前修掉：① 拒单/部分成交静默破坏仓位路径；② 数量按当前价重算使仓位随路径复利；③ 容量政策与场所报告成交量耦合（Aster 日均 BTCUSDT 成交量中位数 9,959 vs Binance 128,882）。这已提升为 Track B 的**首道门禁 B0**。
...
| 执行经济性（Track B） | 🔴 Red | B0 未关闭：三场所结果矛盾且极差 2.7× 本金，拒单/复利/容量三放大器未修 |
| 模型闸门能力（Track A） | 🟡 Amber | 网关/可逆过滤/A4 已验证，但真实 checkpoint 提议 0 删除、`MAX_SCORED=32` 长上下文整体 bypass、Catalog Choice 受干扰 100%→54.17% |
...
**执行**（按序五个子项）：
0. **B0-0 冻结可复现基线**：固定 PIT manifest、checkpoint 哈希、种子、交易窗口、费率、容量参数与 run id；同一输入重复 replay 必须产生相同事件摘要与 PnL。
1. **B0-A 订单/成交状态机**：不可变 order/fill 事件账本（订单、子成交、数量、价格、时间、状态、残余、费用、场所原因码）；拒单成交量必须为 0；部分成交不得超过请求量；残余只能被后续事件消化；重复事件幂等；账本现金/仓位守恒。**禁止**从假定仓位继续、禁止以 bar-end 汇总替代逐事件状态。
2. **B0-B 修正路径复利仓位**：锁定 decision-time sizing 快照；按实际成交、费用、滑点与剩余仓位更新权益；仓位规模默认声明式且与路径无关，复利只能作为显式且单独报告的选项。
3. **B0-C 显式建模 capacity × volume**：参与率、盘口深度代理、ADV、最小步长、延迟与冲击进入每 venue/side/时间桶的成交函数；做 0.25x/0.5x/1x/2x 成交量压力测试；不得出现超过可用深度的成交。不得用固定 capacity 系数做场所排名或风险结论。
4. **B0-D 集成回放 + PnL 归因**：把 PnL 拆为信号/价格、点差、手续费、滑点、拒单、部分成交、路径仓位与容量影响，全部回到账本；三场所同输入重放，归因之和与最终 PnL 对账；存在未解释残差时不得输出头条数字。另：**强制敏感性报告**（扰动种子 × 日期集 × 场所的离散度），单个数字不得作为结果呈现。
