...
**价格水平差异不足以解释这个极差**——但这不排除数据问题：时间对齐、单位、资金费处理、成交量口径与缺测尚未逐一排除审计。已定位的**候选机制**在执行路径上，必须在任何金融数字有意义之前修掉：① 拒单/部分成交静默改变成交尺寸与时机（仓位水平因目标仓位语义自愈，但路径不自愈）；② 容量政策与场所报告成交量耦合（`10%×bar volume`，Aster 日均 BTCUSDT 成交量中位数 9,959 vs Binance 128,882——Aster 27 笔部分成交的确认来源）。注：此前所称"仓位按权益复利"经代码核实**不成立**（sizing 用的是 initial_cash），已在 roadmap B0-B 更正。这已提升为 Track B 的**首道门禁 B0**。
...
| 执行经济性（Track B） | 🔴 Red | B0 未关闭：三场所结果矛盾且极差 2.7× 本金，拒单/容量两大已确认放大器未修 |
| 模型闸门能力（Track A） | 🟡 Amber | 网关/可逆过滤/A4 已验证，但真实 checkpoint 提议 0 删除、`MAX_SCORED=32` 长上下文整体 bypass、Catalog Choice 受干扰 100%→54.17%；**skill 实测：默认 0.9 阈值下 13/13 全弃权，降阈值后 3/6=50% 且错答恰是置信度最高三个**（见 [skill 就绪度实测](NANOJEV_SKILL_READINESS_V1.md)） |
...
| **本地 skill 就绪度实测** | ✅ 已测（2026-09-19） | **技术可用（按需启动/确定性/离线/0.27–0.33s）、决策不可用（默认阈值 13/13 弃权；降阈值 3/6=50% 且置信度与正确性反相关）**；历史 6/6 反馈均为 `fallback`。见 [skill 就绪度实测](NANOJEV_SKILL_READINESS_V1.md) |
