...
| **域适应 runbook** | ✅ 交付（未训练） | 三个已验证阻塞：**A 语料不存在**（构建器自检失败后被其所有者移除）、**B 无语料→训练器适配器**、**C 协议 §7（LoRA+BF16）本机不可执行**（无 peft、MPS 拒 BF16）→ 全骨干 FP32 属**协议变更需重新评审**。算力实测便宜：**1.41 s/步**、峰值 **5.26 GB**、300 步约 **8–12 分钟**。已提出 G0–G5 验收门限与回滚路径 |
