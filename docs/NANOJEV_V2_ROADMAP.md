...
| B0 (T1–T4) | Frozen protocol with verified digest, order-outcome divergence audit, locked path-independent sizing, decoupled capacity | **Found and fixed a window defect that invalidated every earlier number**; cross-venue spread 271,945 → **3,964**; the "chaotic" conclusion retracted |
| Skill measurement + hardening | Abstention measured on 13 engineering questions; scope guard; out-of-domain diagnosis; attribution probe; temperature study | 476 tests; guard `nanojev-scope-guard-v1` (17 tests); **3 of the author's own claims withdrawn by an adversarial pass** |
...
See [laya and reflex on a Mac](LAYA_REFLEX_MAC_STUDY_V1.md).
...
| T9 | A2 gate model that proposes drops (contrastive curation protocol) | A | L | review gate | 🟡 corpus delivered: **41 pairs / 246 items**, 7 families, splits 108/42/42/54, 25 tests, every row through the real trainer validator. **Below the 120-pair target**; the author declined to pad it. Architecture question (T9a) open |
