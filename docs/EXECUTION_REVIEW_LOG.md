...
策略跟踪目标仓位但下**绝对数量**单，因此**被拒或部分成交**会静默破坏既定仓位路径，随后策略基于它并不持有的仓位继续复利。三个场所分别产生 **0 / 2 / 27** 次此类事件。任何推断之前，必须先让 harness 把"实际成交 ≠ 预期成交"当作**显式状态错误**（或改用只减仓的 `position_effect` 语义）。

第二个放大器：数量每次按当前价重算（`0.25 × equity × leverage / price`），因此仓位规模随路径**复利**而非固定。
